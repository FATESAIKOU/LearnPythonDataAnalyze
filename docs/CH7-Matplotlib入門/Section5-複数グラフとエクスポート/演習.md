# Section5: 複数グラフとエクスポート - 演習問題

## 📝 基本演習

---

### 演習1: 2x2のグリッド

`plt.subplots(2, 2)` を使って4つのグラフを配置してください。

**要件:**
- 左上: 折れ線グラフ
- 右上: 棒グラフ
- 左下: 散布図
- 右下: ヒストグラム

```python
import matplotlib.pyplot as plt

x = [1, 2, 3, 4, 5]
y = [10, 25, 20, 35, 30]

# 2x2のグリッドを作成
# ここにコードを書く

plt.tight_layout()
plt.show()
```

**期待される出力:**

2行2列のグリッドに折れ線グラフ、棒グラフ、散布図、ヒストグラムがそれぞれ配置されます：

![2x2グリッドレイアウト](images/exercise_1_2x2.png)

<details>
<summary>解答例を見る</summary>

```python
import matplotlib.pyplot as plt

x = [1, 2, 3, 4, 5]
y = [10, 25, 20, 35, 30]

# 2x2のグリッドを作成
fig, axes = plt.subplots(2, 2, figsize=(12, 10))

# 左上: 折れ線グラフ
axes[0, 0].plot(x, y, 'b-o')
axes[0, 0].set_title('Line Chart')

# 右上: 棒グラフ
axes[0, 1].bar(x, y, color='orange')
axes[0, 1].set_title('Bar Chart')

# 左下: 散布図
axes[1, 0].scatter(x, y, s=100, c='green')
axes[1, 0].set_title('Scatter Plot')

# 右下: ヒストグラム
axes[1, 1].hist(y, bins=5, color='purple', edgecolor='black')
axes[1, 1].set_title('Histogram')

plt.tight_layout()
plt.show()
```

**解説:**
- `axes[行, 列]` で各サブプロットにアクセス
- 各 axes に対して通常のプロット関数を使用

</details>

---

### 演習2: 1行3列のレイアウト

`plt.subplots(1, 3)` を使って横に3つ並べてください。

```python
import matplotlib.pyplot as plt

categories = ['A', 'B', 'C', 'D']
values1 = [10, 20, 15, 25]
values2 = [15, 25, 20, 30]
values3 = [12, 22, 18, 28]

# 1行3列のグリッドを作成
# ここにコードを書く

plt.tight_layout()
plt.show()
```

**期待される出力:**

横に3つ並んだ年度別の棒グラフが表示されます：

![1行3列レイアウト](images/exercise_2_1x3.png)

<details>
<summary>解答例を見る</summary>

```python
import matplotlib.pyplot as plt

categories = ['A', 'B', 'C', 'D']
values1 = [10, 20, 15, 25]
values2 = [15, 25, 20, 30]
values3 = [12, 22, 18, 28]

# 1行3列のグリッドを作成
fig, axes = plt.subplots(1, 3, figsize=(15, 5))

# 1次元配列なので [0], [1], [2] でアクセス
axes[0].bar(categories, values1, color='steelblue')
axes[0].set_title('2022')
axes[0].set_ylabel('Sales')

axes[1].bar(categories, values2, color='coral')
axes[1].set_title('2023')

axes[2].bar(categories, values3, color='seagreen')
axes[2].set_title('2024')

plt.tight_layout()
plt.show()
```

**解説:**
- 1行または1列の場合、axes は1次元配列
- `axes[0]`, `axes[1]`, `axes[2]` でアクセス

</details>

---

### 演習3: x軸を共有

`sharex=True` を使って、x軸を共有する2つのグラフを作成してください。

```python
import matplotlib.pyplot as plt

months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun']
sales = [100, 120, 110, 140, 150, 160]
profit = [15, 20, 18, 25, 28, 30]

# x軸を共有する2行1列のグリッドを作成
# ここにコードを書く

plt.tight_layout()
plt.show()
```

**期待される出力:**

x軸（月）を共有する上下2つのグラフで、売上と利益のトレンドを比較できます：

![x軸を共有したグラフ](images/exercise_3_sharex.png)

<details>
<summary>解答例を見る</summary>

```python
import matplotlib.pyplot as plt

months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun']
sales = [100, 120, 110, 140, 150, 160]
profit = [15, 20, 18, 25, 28, 30]

# x軸を共有する2行1列のグリッドを作成
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8), sharex=True)

# 上段: 売上
ax1.plot(months, sales, 'b-o', linewidth=2, markersize=8)
ax1.set_ylabel('Sales (万円)', fontsize=12)
ax1.set_title('Sales and Profit Trend', fontsize=14)
ax1.grid(True, alpha=0.3)

# 下段: 利益
ax2.plot(months, profit, 'g-s', linewidth=2, markersize=8)
ax2.set_ylabel('Profit (万円)', fontsize=12)
ax2.set_xlabel('Month', fontsize=12)
ax2.grid(True, alpha=0.3)

plt.tight_layout()
plt.show()
```

**解説:**
- `sharex=True` でx軸を共有
- 下のグラフにのみ x軸ラベルを設定

</details>

---

### 演習4: y軸を共有

`sharey=True` を使って、y軸を共有する2つの棒グラフを作成してください。

```python
import matplotlib.pyplot as plt

categories = ['A', 'B', 'C', 'D']
values_2023 = [80, 100, 90, 120]
values_2024 = [90, 110, 100, 130]

# y軸を共有する1行2列のグリッドを作成
# ここにコードを書く

plt.tight_layout()
plt.show()
```

<details>
<summary>解答例を見る</summary>

```python
import matplotlib.pyplot as plt

categories = ['A', 'B', 'C', 'D']
values_2023 = [80, 100, 90, 120]
values_2024 = [90, 110, 100, 130]

# y軸を共有する1行2列のグリッドを作成
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5), sharey=True)

ax1.bar(categories, values_2023, color='steelblue')
ax1.set_title('2023', fontsize=14)
ax1.set_ylabel('Sales', fontsize=12)

ax2.bar(categories, values_2024, color='coral')
ax2.set_title('2024', fontsize=14)

# 全体のタイトル
fig.suptitle('Year-over-Year Comparison', fontsize=16, fontweight='bold')

plt.tight_layout()
plt.subplots_adjust(top=0.88)
plt.show()
```

**解説:**
- `sharey=True` でy軸を共有
- 左のグラフにのみ y軸ラベルを設定

</details>

---

### 演習5: 双軸グラフ

棒グラフ（売上）と折れ線グラフ（利益率）を **同じグラフ** に描いてください。

```python
import matplotlib.pyplot as plt

months = ['Jan', 'Feb', 'Mar', 'Apr', 'May']
sales = [100, 120, 110, 130, 150]
profit_rate = [10, 12, 11, 14, 16]

# 双軸グラフを作成
# ここにコードを書く

plt.show()
```

**期待される出力:**

左軸に売上（棒グラフ）、右軸に利益率（折れ線グラフ）を配置した双軸グラフが表示されます：

![双軸グラフ](images/exercise_5_twinx.png)

<details>
<summary>解答例を見る</summary>

```python
import matplotlib.pyplot as plt

months = ['Jan', 'Feb', 'Mar', 'Apr', 'May']
sales = [100, 120, 110, 130, 150]
profit_rate = [10, 12, 11, 14, 16]

fig, ax1 = plt.subplots(figsize=(10, 6))

# 左軸: 売上（棒グラフ）
color1 = 'steelblue'
ax1.bar(months, sales, color=color1, alpha=0.7, label='Sales')
ax1.set_xlabel('Month', fontsize=12)
ax1.set_ylabel('Sales (万円)', color=color1, fontsize=12)
ax1.tick_params(axis='y', labelcolor=color1)

# 右軸を作成
ax2 = ax1.twinx()

# 右軸: 利益率（折れ線グラフ）
color2 = 'coral'
ax2.plot(months, profit_rate, 'o-', color=color2, linewidth=2, 
         markersize=8, label='Profit Rate')
ax2.set_ylabel('Profit Rate (%)', color=color2, fontsize=12)
ax2.tick_params(axis='y', labelcolor=color2)

# タイトル
plt.title('Sales and Profit Rate', fontsize=14, fontweight='bold')

# 凡例をまとめる
lines1, labels1 = ax1.get_legend_handles_labels()
lines2, labels2 = ax2.get_legend_handles_labels()
ax1.legend(lines1 + lines2, labels1 + labels2, loc='upper left')

plt.tight_layout()
plt.show()
```

**解説:**
- `ax1.twinx()` で右側にy軸を追加
- 凡例は両方の軸からまとめて表示

</details>

---

### 演習6: PNG形式で保存

グラフを **300dpi の PNG形式** で保存してください。

```python
import matplotlib.pyplot as plt

x = [1, 2, 3, 4, 5]
y = [10, 25, 20, 35, 30]

plt.figure(figsize=(10, 6))
plt.plot(x, y, 'b-o', linewidth=2)
plt.title('Sample Chart')
plt.xlabel('X')
plt.ylabel('Y')
plt.grid(True, alpha=0.3)

# 300dpi の PNG で保存
# ここにコードを書く

plt.show()
```

**期待される出力:**

以下のようなシンプルな折れ線グラフがPNG形式で保存されます：

![PNG保存サンプル](images/exercise_6_png.png)

<details>
<summary>解答例を見る</summary>

```python
import matplotlib.pyplot as plt

x = [1, 2, 3, 4, 5]
y = [10, 25, 20, 35, 30]

plt.figure(figsize=(10, 6))
plt.plot(x, y, 'b-o', linewidth=2)
plt.title('Sample Chart')
plt.xlabel('X')
plt.ylabel('Y')
plt.grid(True, alpha=0.3)

# 300dpi の PNG で保存
plt.savefig('sample_chart.png', 
            dpi=300, 
            bbox_inches='tight', 
            facecolor='white')

plt.show()
```

**解説:**
- `dpi=300`: 印刷品質
- `bbox_inches='tight'`: 余白を最小化
- `facecolor='white'`: 背景を白に

</details>

---

### 演習7: 複数形式で保存

同じグラフを **PNG, PDF, SVG** の3形式で保存してください。

```python
import matplotlib.pyplot as plt

x = [1, 2, 3, 4, 5]
y = [10, 25, 20, 35, 30]

fig, ax = plt.subplots(figsize=(10, 6))
ax.plot(x, y, 'b-o', linewidth=2)
ax.set_title('Multi-Format Export')

# 3形式で保存
# ここにコードを書く

plt.show()
```

**期待される出力:**

以下のようなグラフが3つの形式（PNG、PDF、SVG）で保存されます：

![複数形式エクスポート](images/exercise_7_multi.png)

<details>
<summary>解答例を見る</summary>

```python
import matplotlib.pyplot as plt

x = [1, 2, 3, 4, 5]
y = [10, 25, 20, 35, 30]

fig, ax = plt.subplots(figsize=(10, 6))
ax.plot(x, y, 'b-o', linewidth=2)
ax.set_title('Multi-Format Export')

# PNG（ラスター形式、Web向け）
fig.savefig('chart.png', dpi=150, bbox_inches='tight', facecolor='white')

# PDF（ベクター形式、印刷向け）
fig.savefig('chart.pdf', bbox_inches='tight')

# SVG（ベクター形式、Web向け）
fig.savefig('chart.svg', bbox_inches='tight')

plt.show()
```

**解説:**
- PNG: ピクセル画像（Web、プレゼン向け）
- PDF: ベクター画像（印刷、論文向け）
- SVG: ベクター画像（Web、拡大しても綺麗）

</details>

---

### 演習8: GridSpec で不均等レイアウト

以下のようなレイアウトを作成してください。

```
┌───────────────────────┐
│    大きいグラフ       │  ← 上段全幅
├───────────┬───────────┤
│  小さい1  │  小さい2  │  ← 下段2分割
└───────────┴───────────┘
```

```python
import matplotlib.pyplot as plt
import matplotlib.gridspec as gridspec

x = [1, 2, 3, 4, 5]
y = [10, 20, 15, 25, 20]

# GridSpec でレイアウトを作成
# ここにコードを書く

plt.tight_layout()
plt.show()
```

**期待される出力:**

GridSpecを使用して上段に大きなグラフ、下段に2つの小さなグラフを配置した不均等レイアウトが表示されます：

![GridSpecによる不均等レイアウト](images/exercise_8_gridspec.png)

<details>
<summary>解答例を見る</summary>

```python
import matplotlib.pyplot as plt
import matplotlib.gridspec as gridspec

x = [1, 2, 3, 4, 5]
y = [10, 20, 15, 25, 20]

fig = plt.figure(figsize=(12, 8))

# 2行2列のグリッド
gs = gridspec.GridSpec(2, 2, height_ratios=[2, 1])

# 上段: 2列分を使用
ax1 = fig.add_subplot(gs[0, :])  # 0行目、全列
ax1.plot(x, y, 'b-o', linewidth=2)
ax1.set_title('Main Chart (Full Width)', fontsize=14)

# 下段左
ax2 = fig.add_subplot(gs[1, 0])
ax2.bar(x, y, color='orange')
ax2.set_title('Sub Chart 1')

# 下段右
ax3 = fig.add_subplot(gs[1, 1])
ax3.scatter(x, y, s=100, c='green')
ax3.set_title('Sub Chart 2')

plt.tight_layout()
plt.show()
```

**解説:**
- `gs[0, :]` で0行目の全列を指定
- `height_ratios=[2, 1]` で上段を下段の2倍の高さに

</details>

---

## ✅ 演習のまとめ

| 演習 | 学んだこと |
|------|------------|
| 1 | 2x2グリッドの基本 |
| 2 | 1行複数列のレイアウト |
| 3 | x軸の共有 |
| 4 | y軸の共有 |
| 5 | 双軸グラフ |
| 6 | PNG保存 |
| 7 | 複数形式での保存 |
| 8 | GridSpec |

---

## 🔗 次のステップ

これらの基本を理解したら、「応用問題」で実践的な課題に挑戦しましょう！
