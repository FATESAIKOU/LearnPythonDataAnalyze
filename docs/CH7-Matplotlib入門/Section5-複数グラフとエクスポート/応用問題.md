...
3. 左下: 製品別売上の円グラフ
4. 右下: 売上と利益の双軸グラフ
5. 全体タイトル: "Sales Dashboard - H1 2024"
6. PNG(300dpi)とPDFで保存

**期待される出力:**

2x2のグリッドに売上vs目標の折れ線グラフ、製品別売上の棒グラフ、円グラフ、双軸グラフを配置したダッシュボードが表示されます：

![売上分析ダッシュボード](images/app_dashboard.png)
...
以下のデータで、**x軸を共有した3段グラフ** を作成してください。
...
2. 中段: 売上（折れ線グラフ、緑）
3. 下段: 転換率（折れ線グラフ、赤）
4. x軸を共有（一番下のグラフにのみラベル）
5. 各グラフにグリッドを追加

**期待される出力:**

x軸を共有した3段グラフで、来客数（棒グラフ）、売上（折れ線）、転換率（折れ線）を同じ時間軸で比較できます：

![時系列比較ダッシュボード](images/app_timeseries.png)
...
**解説:**
- `sharex=True` で3つのグラフがx軸を共有
- `fill_between()` で折れ線の下を塗りつぶし
...
**GridSpec** を使って以下のレイアウトを作成してください。
...
GridSpecを使用して、上段に大きなメインチャート、下段に3つの小さなグラフを配置した不均等レイアウトが表示されます：

![不均等レイアウト](images/app_uneven_layout.png)
...
ax_main.set_ylabel('Sales (万円)', fontsize=12)
...
**解説:**
- `gs[0, :]`: 0行目の全列
- `height_ratios=[2, 1]`: 上段が下段の2倍の高さ

</details>

---

### 応用問題4: 複数形式でのバッチエクスポート

同じデータから複数のグラフを作成し、それぞれを **適切な形式** で保存してください。
...
関数を使用して作成されたプロフェッショナルな折れ線グラフが、PNG、PDF、SVGの3形式で保存されます：

![バッチエクスポート](images/app_batch_export.png)
...
以下のデータを使って、**プロフェッショナルな分析レポート** を作成してください。
...
**要件:**
1. GridSpec で不均等レイアウト（上段1つ + 下段2つ + 最下段2つ）
2. 上段: 収益・コスト・利益の推移（双軸：棒+折れ線）
3. 左下: 製品別収益（横棒グラフ）
4. 右下: 製品別シェア（円グラフ）
5. 左最下: 利益率推移（面グラフ）
6. 右最下: 地域別分布（箱ひげ図）
...
GridSpecを使用した5つのグラフで構成されたプロフェッショナルなビジネス分析レポートが表示されます。収益・コスト・利益の推移、製品別収益、製品シェア、利益率推移、地域別分布を一目で把握できます：

![総合レポート](images/app_full_report.png)
...
ax1.set_ylabel('Amount (万円)', fontsize=11)
ax1.set_xticks(x)
...
ax1_twin.set_ylabel('Profit (万円)', color=colors[2], fontsize=11)
...
ax2.set_xlabel('Revenue (万円)', fontsize=11)
...
| 問題 | テーマ | ポイント |
|------|--------|----------|
| 1 | ダッシュボード | 2x2レイアウト + 双軸 |
| 2 | 時系列比較 | 共有x軸の活用 |
| 3 | 不均等レイアウト | GridSpec |
| 4 | バッチエクスポート | 関数化と複数形式 |
| 5 | 総合レポート | 全技術の統合 |
...
これで「CH7: Matplotlib入門」の全セクションが完了しました！

**学んだこと:**
- Matplotlibの基本概念（Figure, Axes）
- 折れ線グラフ、散布図、棒グラフ、ヒストグラム、円グラフ、箱ひげ図
- グラフの装飾（タイトル、凡例、グリッド、注釈）
- 複数グラフのレイアウト（subplots, GridSpec）
- 様々な形式でのエクスポート

次は「CH8: 総合演習」で、これまで学んだ全ての知識を使った実践課題に挑戦します！
